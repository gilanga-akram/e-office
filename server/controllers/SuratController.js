...
class SuratController {
    ...
    static async changeStatus(req, res, next) {
        try {
            const { role, bagian } = req.UserData;
            const { suratId } = req.params;
            const { status } = req.body;
            if (role === 'admin') throw createHttpError(StatusCodes.FORBIDDEN, 'admin cannot edit surat');
            if (bagian !== 'Direktur UPT Lapkesda' && bagian !== 'Kepala Subbag Tata Usaha') {
                throw createHttpError(StatusCodes.FORBIDDEN, 'bukan bagian anda')
            }
            const suratData = await surat.findOne({
                where: {
                    id: suratId,
                },
            });
            if (!suratData) throw createHttpError(StatusCodes.NOT_FOUND, 'surat tidak ditemukan');
            if (suratData.tipe !== 'surat cuti' && suratData.tipe !== 'disposisi') {
                throw createHttpError(StatusCodes.BAD_REQUEST, 'tipe dan jenis surat salah');
            }
            await surat.update({
                status: status,
            }, {
